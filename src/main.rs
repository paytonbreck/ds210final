...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut country_movies: HashMap<String, HashSet<String>> = HashMap::new();
    ...
    for result in csv_reader.deserialize::<NetflixEntry>() {
        ...
        if let Some(title) = record.title {
            ...
            if let Some(country) = record.country {
                country_movies.entry(country.clone()).or_insert(HashSet::new()).insert(title.clone());
                total_movie_counts.entry(country.clone()).and_modify(|c| *c += 1).or_insert(1);
                if let Some(listed_in) = record.listed_in {
                    if listed_in.contains("Horror") {
                        horror_movie_counts.entry(country.clone()).and_modify(|c| *c += 1).or_insert(1);
                    }
                }
            }
        }
    ...
    for movies in country_movies.values() {
    ...
    for (country, horror_count) in &horror_movie_counts {
        if let Some(total_count) = total_movie_counts.get(country) {
            let ratio = *horror_count as f64 / *total_count as f64;
            horror_movie_ratios.insert(country.clone(), ratio);
        }
    }
    ...
    let categories: Vec<&str> = sorted_ratios.iter().map(|(country, _)| country.as_str()).collect();
    ...
    let root = BitMapBackend::new("Horror_Movies_by_Country.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE)?;

    let mut chart = ChartBuilder::on(&root)
        .caption("Ratio of Horror Movies by Country", ("Arial", 20).into_font())
        .build_cartesian_2d(
